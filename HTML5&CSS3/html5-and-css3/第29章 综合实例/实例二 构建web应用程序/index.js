...
function btnAdd_onclick()
{
	...
	tx.oncomplete=function()
	{
		if(chkErrorMsg!="") alert(chkErrorMsg);
		else{
			alert('追加数据成功！');
			showAllData(false);
			btnNew_onclick();
	...
	req.onsuccess=function()
	{
		...
		if(cursor) chkErrorMsg="输入的订单编号在数据库中已存在";
        else{
        	var value={
        		code:data.Code,date:data.Date,goodscode:data.GoodsCode,
        		brandName:data.BrandName,num:data.Num,price:data.Price,
        		personName:data.PersonName,email:data.Email
        	};
        	store.put(value);
        }
	}
